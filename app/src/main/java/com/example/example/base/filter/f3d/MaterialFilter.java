...
//                        光源的位置
                    ...
                    "//环境光的计算\n" +
                    "vec4 ambientColor(){\n" +
                    "    vec3 ambient = uAmbientStrength * uLightColor * ambient;\n" +
                    ...
                    "    float diff=pow(max(dot(normal,hafVector),0.0),32.0);\n" +
                    "    //材质的镜面反射系数*max(0,cos(反射向量与观察向量夹角)^粗糙度*光照颜色\n" +
                    "    //材质的镜面反射系数*max(0,cos(半向量与法向量的夹角)^粗糙度*光照颜色\n" +
                    "    vec3 specular=uSpecularStrength*diff*uLightColor*diffuse*materialspecular;\n" +
    ...
    public void create() {
        super.create();
        materialAmbient = GLES20.glGetUniformLocation(mProgram, "ambient");
        materialDiffuse = GLES20.glGetUniformLocation(mProgram, "diffuse");
...
//        lightingShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);
        GLES20.glUniform3f(materialAmbient, 0.2f, 0.2f, 0.2f);
